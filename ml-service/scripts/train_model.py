...
FEATURES = ["disaster_type", "country", "region", "year", "month", "duration_days"]
TARGET = "severity_class"
CATEGORICAL = ["disaster_type", "country", "region"]
NUMERICAL = ["year", "month", "duration_days"]
...
def main():
    ...
    selected = max(results, key=lambda item: item["weighted_f1"])
    ...
    summary = {
        ...
        "features": FEATURES,
        "target": TARGET,
        "all_models": [{"model": item["model"], "accuracy": item["accuracy"], "weighted_f1": item["weighted_f1"]} for item in results],
    }
