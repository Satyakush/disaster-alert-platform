...
const allowedUpdateFields = ["title", "description", "disasterType", "severity", "status", "location", "coordinates", "radius", "affectedArea", "impact", "source", "urgency", "certainty", "effectiveAt", "expiresAt", "instructions", "risk", "actualOutcome"];

const pickFields = (body) => Object.fromEntries(allowedUpdateFields.filter((field) => Object.prototype.hasOwnProperty.call(body, field)).map((field) => [field, body[field]]));
...
export const createAlert = async (req, res) => {
  try {
    const { title, description, disasterType, severity, status, location, coordinates, radius, affectedArea, impact, source, urgency, certainty, effectiveAt, expiresAt, instructions, risk } = req.body;
    if (!title || !description || !disasterType || !location) return res.status(400).json({ message: "Title, description, disaster type, and location are required" });

    const alert = await Alert.create({ title, description, disasterType, severity, status, location, coordinates, radius, affectedArea, impact, source, urgency, certainty, effectiveAt, expiresAt, instructions, risk, createdBy: req.user.id });
...
export const updateAlert = async (req, res) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(updates, "actualOutcome")) {
      if (req.user.role !== "admin") return res.status(403).json({ message: "Admin access required to record alert outcomes" });
      updates.actualOutcome = { ...updates.actualOutcome, recordedAt: new Date(), recordedBy: req.user.id };
    }
...
export const updateAlertStatus = async (req, res) => {
  try {
    ...
    const alert = await Alert.findByIdAndUpdate(req.params.id, { status }, { new: true, runValidators: true }).populate("createdBy", "name email role");
    if (!alert) return res.status(404).json({ message: "Alert not found" });
