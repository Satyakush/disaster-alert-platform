...
export default function AlertCard({ alert, setAlerts, onSelect, selected }) {
  ...
  return (
    <div className={`ui-alert-card rounded-2xl border bg-white p-5 shadow-sm transition ${selected ? "border-red-400 ring-2 ring-red-100" : "border-slate-200 hover:shadow-md"}`}>
      <button type="button" onClick={onSelect} className="block w-full text-left">
        <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Live disaster alert</p><h3 className="mt-1 text-lg font-bold tracking-tight text-slate-900">{alert.title}</h3></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold capitalize text-slate-600">{alert.status}</span></div>
