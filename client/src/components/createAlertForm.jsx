...
export default function CreateAlertForm({ onCreated, selectedRegion }) {
  ...
  return (
    <div className="ui-command-card rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-6 border-b border-slate-100 pb-5"><div className="flex items-center gap-3"><div className="rounded-xl bg-red-50 p-2.5 text-red-600">🚨</div><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-red-600">Alert operations</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Create New Alert</h2><p className="mt-1 text-sm text-slate-500">Select a location on the map, assess the hazard scenario, then publish a structured disaster alert.</p></div></div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"><select name="disasterType" value={form.disasterType} onChange={handleChange} required className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-400"><option value="">Disaster type</option>{disasterTypes.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select name="severity" value={form.severity} onChange={handleChange} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-400">{severities.map((value) => <option key={value} value={value}>{value[0].toUpperCase() + value.slice(1)} severity</option>)}</select><select name="status" value={form.status} onChange={handleChange} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-400">{statuses.map((value) => <option key={value} value={value}>{value[0].toUpperCase() + value.slice(1)} status</option>)}</select><input type="number" name="radius" min="100" max="100000" step="100" value={form.radius} onChange={handleChange} placeholder="Radius in meters" required className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400" /></div>
        ...
        <div className="ui-intelligence-panel rounded-2xl border border-cyan-100 bg-cyan-50/40 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-semibold text-slate-800">AI risk assessment</h3><p className="text-xs text-slate-500">Calculate explainable risk before publishing this alert.</p></div><button type="button" onClick={assessRisk} disabled={riskLoading || !selectedRegion || !form.disasterType} className="rounded-lg bg-cyan-600 px-3 py-2 text-xs font-semibold text-white hover:bg-cyan-700 disabled:cursor-not-allowed disabled:opacity-50">{riskLoading ? "Assessing..." : "Assess risk"}</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["intensity", "Intensity"], ["probability", "Probability"], ["exposure", "Exposure"], ["vulnerability", "Vulnerability"], ["historicalRisk", "Historical risk"]].map(([name, label]) => <label key={name} className="text-xs font-medium text-slate-600">{label}<div className="mt-1 flex items-center gap-2"><input type="range" min="0" max="100" value={riskInputs[name]} onChange={(event) => updateRisk(name, event.target.value)} className="w-full" /><span className="w-7 text-right font-semibold text-slate-800">{riskInputs[name]}</span></div></label>)}</div>{riskError && <p className="mt-3 text-xs text-red-600">{riskError}</p>}{riskResult && <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-white p-3"><p className="text-[11px] uppercase text-slate-400">Risk score</p><p className="mt-1 text-xl font-bold text-slate-900">{riskResult.riskScore}/100</p></div><div className="rounded-lg bg-white p-3"><p className="text-[11px] uppercase text-slate-400">Risk level</p><p className="mt-1 text-xl font-bold capitalize text-slate-900">{riskResult.riskLevel}</p></div><div className="rounded-lg bg-white p-3"><p className="text-[11px] uppercase text-slate-400">Confidence</p><p className="mt-1 text-xl font-bold text-slate-900">{Math.round((riskResult.confidence || 0) * 100)}%</p></div></div>}</div>
        <div className="ui-section-panel rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="mb-3"><h3 className="text-sm font-semibold text-slate-800">Impact & infrastructure estimate</h3><p className="text-xs text-slate-500">Record the estimated exposure for this alert.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["population", "Population"], ["households", "Households"], ["hospitals", "Hospitals"], ["schools", "Schools"], ["evacuationCenters", "Evacuation centers"], ["roadsKm", "Roads affected (km)"], ["estimatedDamage", "Estimated damage"]].map(([name, label]) => <label key={name} className="text-xs font-medium text-slate-500">{label}<input type="number" min="0" step="0.1" name={name} value={form.impact[name]} onChange={handleImpactChange} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none focus:border-cyan-500" /></label>)}</div></div>
