...
export default function MapView({ onRegionSelect, alerts = [], reports = [], shelters = [] }) {
  ...
  const alertPoints = alerts.filter(
    (alert) => alert.coordinates?.coordinates?.length === 2 && ["active", "escalated"].includes(alert.status)
  );
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-col gap-2 sm:flex-row">
        ...
        />
        <button onClick={handleSearch} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
          Search
        </button>
      </div>
      ...
      {region && (
        <div className="mb-3 text-sm text-slate-600">
          <div className="flex justify-between">
            <span>Selected risk radius</span>
            <span className="font-semibold">{region.radius / 1000} km</span>
          </div>
          <input
            type="range"
            min="1000"
            ...
            step="500"
            value={region.radius}
            onChange={(e) => setRegion({ ...region, radius: Number(e.target.value) })}
            className="w-full"
          />
        </div>
      )}
      ...
      <div className="mb-3 flex flex-wrap gap-3 text-xs text-slate-500">
        <span>● Active alerts: {alertPoints.length}</span>
        <span>● Reports: {reportPoints.length}</span>
        <span>● Shelters: {shelterPoints.length}</span>
      </div>
      ...
      <MapContainer center={center} zoom={10} style={{ height: "480px", width: "100%", borderRadius: "12px" }}>
        ...
        {region && (
          <Circle center={[region.lat, region.lng]} radius={region.radius} pathOptions={{ color: "#0f172a", fillOpacity: 0.08 }} />
        )}
        ...
        {alertPoints.map((alert) => {
          ...
          return (
            <div key={alert._id}>
              <Circle center={[lat, lng]} radius={Math.max(Number(alert.radius) || 1500, 500)} pathOptions={{ color, fillColor: color, fillOpacity: 0.2, weight: 2 }} />
        ...
        {reportPoints.map((report) => {
          ...
          return <Marker key={`report-${report._id}`} position={[lat, lng]} />;
        })}

        {shelterPoints.map((shelter) => {
          const [lng, lat] = shelter.coordinates.coordinates;
          return <Marker key={`shelter-${shelter._id}`} position={[lat, lng]} />;
        })}
      </MapContainer>
    </div>
  );
