...
const formatType = (value) => value?.replace(/-/g, " ").replace(/w/g, (char) => char.toUpperCase());
...
export default function ReportReview() {
  ...
  useEffect(() => {
    if (!selectedReportId || loading) return;
    const target = reportRefs.current[selectedReportId];
    if (!target) return;

    target.scrollIntoView({ behavior: "smooth", block: "center" });
    target.classList.add("ring-2", "ring-cyan-400");
    const timer = setTimeout(() => target.classList.remove("ring-2", "ring-cyan-400"), 3000);
    return () => clearTimeout(timer);
  }, [selectedReportId, loading, reports]);
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-400">Review citizen observations, verify credible incidents, and promote verified reports into live alerts.</p>
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {reports.map((report) => (
              <article key={report._id} ref={(element) => { reportRefs.current[report._id] = element; }} className="admin-review-card rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl transition">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${priorityClasses[report.priority] || priorityClasses.medium}`}>{report.priority}</span>
                ...
                {report.mediaUrl && <a href={report.mediaUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm text-cyan-400 hover:text-cyan-300"><ExternalLink size={14} />View attached media</a>}

                {report.status === "pending" && (
                  <>
                    <textarea value={note[report._id] || ""} onChange={(event) => setNote((current) => ({ ...current, [report._id]: event.target.value }))} placeholder="Verification note (optional)" className="mt-5 min-h-20 w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm text-white outline-none focus:border-cyan-500" />
                    <div className="mt-3 flex gap-3">
                      <button type="button" disabled={busyId === report._id} onClick={() => changeStatus(report._id, "verified")} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-slate-950 disabled:opacity-50"><CheckCircle2 size={16} /> Verify</button>
                      <button type="button" disabled={busyId === report._id} onClick={() => changeStatus(report._id, "rejected")} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-red-500/15 px-4 py-2.5 text-sm font-semibold text-red-300 disabled:opacity-50"><XCircle size={16} /> Reject</button>
                    </div>
                  </>
                )}

                {report.status === "verified" && !report.linkedAlert && <button type="button" disabled={busyId === report._id} onClick={() => convertToAlert(report._id)} className="mt-5 w-full rounded-lg bg-cyan-500 px-4 py-2.5 text-sm font-bold text-slate-950 disabled:opacity-50">Convert to Live Alert</button>}

                {report.linkedAlert && <div className="mt-5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-3 text-sm text-emerald-300">Linked alert: {report.linkedAlert.title} · {report.linkedAlert.status}</div>}
              </article>
            ))}
          </div>
        )}
