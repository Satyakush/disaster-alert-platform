...
function currentSignalFromAlert(alert) {
  ...
  return { type: alert.disasterType || "other", intensity: severity.intensity, probability: Math.round((urgency + certainty) / 2), exposure: impactExposure(alert), vulnerability: impactVulnerability(alert), source: "active-alert", alertId: alert._id };
}

function currentSignalFromReport(report) {
  if (!report) return null;
  const severity = severityDefaults[report.priority] || severityDefaults.medium;
  return { type: report.disasterType || report.type || "other", intensity: severity.intensity, probability: severity.probability, exposure: 0, vulnerability: 0, source: "verified-report", reportId: report._id };
}
...
export default function Dashboard() {
  ...
  const handleRegionSelect = (region) => {
    ...
    setRiskResult(null);
    setRiskError("");
    if (currentSignal) setRiskInputs((current) => ({ ...current, type: currentSignal.type, intensity: currentSignal.intensity, probability: currentSignal.probability, exposure: currentSignal.exposure, vulnerability: currentSignal.vulnerability, historicalRisk: 0 }));
    else setRiskInputs((current) => ({ ...current, intensity: 0, probability: 0, exposure: 0, vulnerability: 0, historicalRisk: 0 }));
  };
  ...
  return (
    <div className="admin-modern min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6">
        ...
        <section className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_400px]"><div className="min-w-0 space-y-6"><MapView onRegionSelect={handleRegionSelect} alerts={alerts} reports={reports} shelters={shelters} infrastructure={infrastructure} route={route} /><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-red-600">Live incident feed</p><h2 className="mt-1 text-xl font-bold text-slate-900">Active intelligence</h2><p className="mt-1 text-sm text-slate-500">Live alerts are updated automatically across connected clients.</p></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-3"><FilterSelect value={filters.status} onChange={(value) => setFilters({ ...filters, status: value })} options={["active", "escalated", "resolved", "archived", "draft"]} placeholder="All statuses" /><FilterSelect value={filters.disasterType} onChange={(value) => setFilters({ ...filters, disasterType: value })} options={hazards.map(([value]) => value)} placeholder="All hazards" /><FilterSelect value={filters.severity} onChange={(value) => setFilters({ ...filters, severity: value })} options={["low", "medium", "high", "critical"]} placeholder="All severity" /></div></div>{error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}</div><div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">{filteredAlerts.length === 0 ? <p className="text-sm text-slate-500">No alerts match the selected filters.</p> : (showAllAlerts ? filteredAlerts : filteredAlerts.slice(0, 3)).map((alert) => <AlertCard key={alert._id} alert={alert} setAlerts={setAlerts} onSelect={() => setSelectedAlert(alert)} selected={selectedAlert?._id === alert._id} />)}</div>{filteredAlerts.length > 3 && <div className="border-t border-slate-200 px-5 py-4"><button type="button" onClick={() => setShowAllAlerts((current) => !current)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">{showAllAlerts ? "Show latest 3 alerts" : `View full alerts (${filteredAlerts.length})`}</button></div>}</section></div><div className="min-w-0 lg:sticky lg:top-5"><RiskPanel selectedRegion={selectedRegion} riskInputs={riskInputs} setRiskInputs={setRiskInputs} onPreset={applySeverityPreset} onAnalyze={analyzeSelectedRisk} riskResult={riskResult} riskLoading={riskLoading} riskError={riskError} riskLevel={riskLevel} riskScore={riskScore} /></div></section>
