...
const severityDefaults = {
  low: { intensity: 25, probability: 25 },
  medium: { intensity: 45, probability: 45 },
  high: { intensity: 70, probability: 70 },
  critical: { intensity: 90, probability: 90 },
};
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Multi-hazard risk intelligence</p><h2 className="mt-1 text-xl font-bold text-slate-900">Scenario assessment</h2></div>
      ...
      <div className="mt-5 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <RangeField label="Hazard intensity" value={riskInputs.intensity} onChange={(value) => update("intensity", value)} />
          <RangeField label="Hazard probability" value={riskInputs.probability} onChange={(value) => update("probability", value)} />
          <RangeField label="Population exposure" value={riskInputs.exposure} onChange={(value) => update("exposure", value)} />
          <RangeField label="Vulnerability" value={riskInputs.vulnerability} onChange={(value) => update("vulnerability", value)} />
        ...
        {riskLoading ? <div className="text-sm text-slate-500">Calculating hazard, exposure, vulnerability, and historical risk...</div> : riskResult ? <div className="space-y-5">
          <div><div className="flex items-end justify-between"><span className="text-sm text-slate-500">Risk score</span><span className="text-3xl font-bold text-slate-900">{riskScore}<span className="text-base text-slate-400">/100</span></span></div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-slate-900 transition-all" style={{ width: `${riskScore}%` }} /></div></div>
...
function RangeField({ label, value, onChange }) {
  return <label className="text-sm font-medium text-slate-700"><div className="flex justify-between"><span>{label}</span><span className="font-semibold text-slate-900">{value}</span></div><input type="range" min="0" max="100" value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full" /></label>;
}
