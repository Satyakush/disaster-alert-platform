...
function RiskPanel({ selectedRegion, riskInputs, setRiskInputs, onPreset, onAnalyze, riskResult, riskLoading, riskError, riskLevel, riskScore }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Multi-hazard risk intelligence</p><h2 className="mt-1 text-xl font-bold text-slate-900">Scenario assessment</h2></div>{riskResult && <span className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize ${riskStyles[riskLevel] || riskStyles.low}`}>{riskLevel}</span>}</div><div className="mt-5 space-y-5"><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700">Hazard type<select value={riskInputs.type} onChange={(event) => setRiskInputs((current) => ({ ...current, type: event.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 capitalize outline-none focus:border-slate-400">{hazards.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm font-medium text-slate-700">Scenario severity<select defaultValue="baseline" onChange={(event) => event.target.value !== "baseline" && onPreset(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 capitalize outline-none focus:border-slate-400"><option value="baseline">Custom / baseline</option><option value="low">Low</option><option value="medium">Moderate</option><option value="high">High</option><option value="critical">Extreme</option></select></label></div><div className="grid gap-4 sm:grid-cols-2"><RangeField label="Hazard intensity" value={riskInputs.intensity} onChange={(value) => update("intensity", value)} /><RangeField label="Hazard probability" value={riskInputs.probability} onChange={(value) => update("probability", value)} /><RangeField label="Population exposure" value={riskInputs.exposure} onChange={(value) => update("exposure", value)} /><RangeField label="Vulnerability" value={riskInputs.vulnerability} onChange={(value) => update("vulnerability", value)} /><RangeField label="Historical risk" value={riskInputs.historicalRisk} onChange={(value) => update("historicalRisk", value)} /></div><button onClick={onAnalyze} disabled={!selectedRegion || riskLoading} className="w-full rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60">{riskLoading ? "Analyzing scenario..." : selectedRegion ? "Analyze risk scenario" : "Select an area to analyze"}</button>{riskError && <div className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{riskError}</div>}{riskLoading ? <div className="text-sm text-slate-500">Calculating hazard, exposure, vulnerability, and historical risk...</div> : riskResult ? <div className="space-y-5"><div><div className="flex items-end justify-between"><span className="text-sm text-slate-500">Risk score</span><span className="text-3xl font-bold text-slate-900">{riskScore}<span className="text-base text-slate-400">/100</span></span></div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-slate-900 transition-all" style={{ width: `${riskScore}%` }} /></div></div><div className="flex justify-between text-sm"><span className="text-slate-500">Model confidence</span><span className="font-semibold text-slate-800">{Math.round((riskResult.confidence || 0) * 100)}%</span></div><div><p className="mb-2 text-sm font-semibold text-slate-800">Explainable risk factors</p><div className="space-y-2">{(riskResult.factors || []).map((factor) => <div key={factor.name} className="rounded-lg bg-slate-50 p-3"><div className="flex justify-between gap-3 text-xs"><span className="capitalize text-slate-600">{factor.name.replaceAll("_", " ")}</span><span className="font-semibold text-slate-900">{Number(factor.value).toFixed(0)} · {Number(factor.weight * 100).toFixed(0)}% weight</span></div><div className="mt-1 text-[11px] text-slate-400">Contribution: {Number(factor.contribution).toFixed(1)}</div></div>)}</div></div>{riskResult.recommendedActions?.length > 0 && <div><p className="mb-2 text-sm font-semibold text-slate-800">Recommended actions</p><ul className="space-y-2 text-sm text-slate-600">{riskResult.recommendedActions.slice(0, 4).map((action) => <li key={action}>• {action}</li>)}</ul></div>}</div> : <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">Choose a hazard and scenario inputs, then run the assessment to generate explainable risk intelligence.</div>}</div></div>;
}

function RangeField({ label, value, onChange }) { return <label className="text-sm font-medium text-slate-700"><div className="flex justify-between"><span>{label}</span><span className="font-semibold text-slate-900">{value}</span></div><input type="range" min="0" max="100" value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full" /></label>; }
function StatCard({ icon: Icon, label, value }) { return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><span className="text-sm text-slate-500">{label}</span><Icon size={19} className="text-slate-400" /></div><p className="mt-3 text-3xl font-bold text-slate-900">{value}</p></div>; }
function FilterSelect({ value, onChange, options, placeholder }) { return <select value={value} onChange={(event) => onChange(event.target.value)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none focus:border-slate-400"><option value="">{placeholder}</option>{options.map((option) => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}</select>; }
