...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const handleStatusChanged = (alert) =>
      setAlerts((current) => current.map((item) => (item._id === alert._id ? alert : item)));
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              {riskResult && (
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize ${riskStyles[riskLevel] || riskStyles.low}`}>
                  {riskLevel}
                </span>
              )}
            </div>
            ...
            {!selectedRegion ? (
              <div className="mt-8 rounded-xl bg-slate-50 p-5 text-sm text-slate-500">
                Select an area on the map to calculate its current risk profile.
              </div>
            ) : riskLoading ? (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {(riskResult.factors || []).map((factor) => (
                      <div key={factor.name} className="rounded-lg bg-slate-50 p-3">
                        <div className="flex justify-between gap-3 text-xs">
                          <span className="capitalize text-slate-600">{factor.name.replaceAll("_", " ")}</span>
                          <span className="font-semibold text-slate-900">{Math.round(Number(factor.contribution ?? factor.value ?? 0))}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                {riskResult.recommendedActions?.length > 0 && (
                  <div>
                    <p className="mb-2 text-sm font-semibold text-slate-800">Recommended actions</p>
                    <ul className="space-y-2 text-sm text-slate-600">
                      {riskResult.recommendedActions.slice(0, 4).map((action) => <li key={action}>• {action}</li>)}
                    </ul>
                  </div>
                )}
              </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-xl font-bold text-slate-900">Active intelligence feed</h2>
              <p className="text-sm text-slate-500">Live alerts are updated automatically.</p>
            </div>
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filteredAlerts.length === 0 ? (
              <p className="text-sm text-slate-500">No alerts match the selected filters.</p>
            ) : (
              filteredAlerts.map((alert) => <AlertCard key={alert._id} alert={alert} setAlerts={setAlerts} />)
            )}
          </div>
        </section>
      </main>
