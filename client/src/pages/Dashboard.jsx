    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6">
        <section className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-sm font-medium text-red-600">EARLY WARNING CENTER</p><h1 className="text-3xl font-bold tracking-tight text-slate-900">Disaster Intelligence Dashboard</h1><p className="mt-1 text-sm text-slate-500">Detect hazards, assess risk, and respond faster.</p></div>
          <div className="flex items-center gap-2 text-sm text-emerald-700"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /> Live monitoring connected</div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <MapView onRegionSelect={setSelectedRegion} alerts={alerts} reports={reports} shelters={shelters} route={route} />
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Risk intelligence</p><h2 className="mt-1 text-xl font-bold text-slate-900">Selected area</h2></div>{riskResult && <span className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize ${riskStyles[riskLevel] || riskStyles.low}`}>{riskLevel}</span>}</div>
            {!selectedRegion ? <div className="mt-8 rounded-xl bg-slate-50 p-5 text-sm text-slate-500">Select an area on the map to calculate its current risk profile.</div> : riskLoading ? <div className="mt-8 text-sm text-slate-500">Calculating risk profile...</div> : riskResult ? <div className="mt-6 space-y-5"><div><div className="flex items-end justify-between"><span className="text-sm text-slate-500">Risk score</span><span className="text-3xl font-bold text-slate-900">{riskScore}<span className="text-base text-slate-400">/100</span></span></div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-slate-900 transition-all" style={{ width: `${riskScore}%` }} /></div></div><div className="flex justify-between text-sm"><span className="text-slate-500">Confidence</span><span className="font-semibold text-slate-800">{Math.round((riskResult.confidence || 0) * 100)}%</span></div><div><p className="mb-2 text-sm font-semibold text-slate-800">Risk factors</p><div className="space-y-2">{(riskResult.factors || []).map((factor) => <div key={factor.name} className="rounded-lg bg-slate-50 p-3"><div className="flex justify-between gap-3 text-xs"><span className="capitalize text-slate-600">{factor.name.replaceAll("_", " ")}</span><span className="font-semibold text-slate-900">{Math.round(Number(factor.contribution ?? factor.value ?? 0))}</span></div></div>)}</div></div>{riskResult.recommendedActions?.length > 0 && <div><p className="mb-2 text-sm font-semibold text-slate-800">Recommended actions</p><ul className="space-y-2 text-sm text-slate-600">{riskResult.recommendedActions.slice(0, 4).map((action) => <li key={action}>• {action}</li>)}</ul></div>}</div> : null}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filteredAlerts.length === 0 ? <p className="text-sm text-slate-500">No alerts match the selected filters.</p> : filteredAlerts.map((alert) => <AlertCard key={alert._id} alert={alert} setAlerts={setAlerts} onSelect={() => setSelectedAlert(alert)} selected={selectedAlert?._id === alert._id} />)}</div>
        </section>
      </main>
    </div>
  );
